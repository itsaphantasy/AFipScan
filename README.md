...
- 🚀 **本地实时测速**：TCP + TLS 握手测延迟，结果按延迟从低到高排序，只保留最快的 N 个
...
3. 点【开始测速】→ 约 10~30 秒出结果（运行中再点一次可停止）
...
- **更多优选 IP 源**：自定义候选 IP 拉取网址
- **一键写入**：把测速结果写入 edgetunnel 面板（需先填接口 / 密码 / 订阅）
- **导出结果**：把测速结果保存为文本
